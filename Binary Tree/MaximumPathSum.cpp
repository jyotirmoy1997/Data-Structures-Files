...
int height(Node* root, int &maxi){
    ...
    int lheight = max(0, height(root->left, maxi));
    int rheight = max(0, height(root->right, maxi));
    maxi = max(maxi, lheight + rheight + root->data);

    return root->data + max(lheight, rheight);
}
