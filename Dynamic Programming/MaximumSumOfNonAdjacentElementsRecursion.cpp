...
int maximumSumRecursion(int index, int arr[]){
    ...
    // if index is 0, then we return the current value
    if(index == 0)
        return arr[index];
    
    // if index is -1, then we return 0 as we are not taking the element
    if(index < 0)
        return 0;

    // Picking the element
    int pick = arr[index] + maximumSumRecursion(index - 2, arr);

    // Not Picking the element
    int notPick = 0 + maximumSumRecursion(index - 1, arr);
...
{
    int arr[] = {2, 1, 4, 9};
    cout << maximumSumRecursion(sizeof(arr)/sizeof(int) - 1, arr);
}